...
class PairTradingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.transform = btind.OLS_TransformationN(
            self.data0, self.data1, period=self.p.period
        )
    ...
    def next(self):
        ...
        if (self.zscore[0] > self.upper_limit) and (self.status != 1):
            ...
            # Placing the order
            self.log(
                "SELL CREATE %s, price = %.2f, qty = %d"
                % ("PEP", self.data0.close[0], x + self.qty1)
            )
            self.sell(
                data=self.data0, size=(x + self.qty1)
            )  # Place an order for buying y + qty2 shares
            self.log(
                "BUY CREATE %s, price = %.2f, qty = %d"
                % ("KO", self.data1.close[0], y + self.qty2)
            )
            self.buy(
                data=self.data1, size=(y + self.qty2)
